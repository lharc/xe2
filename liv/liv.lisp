...
(defvar *best-liv-net-0* nil)
(defvar *best-liv-net-1* nil)
(defvar *best-liv-net-2* nil)
(defvar *best-liv-annerror-0* -9999) ; one for each art
(defvar *best-liv-annerror-1* -9999)
(defvar *best-liv-annerror-2* -9999)

(defmacro make-liv (x y) `(list ,x ,y (make-net) 128 (random 3) 0 0 0 nil 0f0 nil 0 0))
...
(defmacro liv-carry    (liv) `(nth 10 ,liv))
(defmacro liv-ox (liv) `(nth 11 ,liv))
(defmacro liv-oy (liv) `(nth 12 ,liv))
(defmacro set-obj (x y i) `(setf (aref *obj* (+ ,x (* ,y *room-mx*))) ,i))
(defmacro get-obj (x y) `(aref *obj* (+ ,x (* ,y *room-mx*))))
(defmacro has-carry (what liv) `(find ,what (liv-carry ,liv)))
(defmacro add-carry (what liv) `(push ,what (liv-carry ,liv)))
(defmacro del-carry (what liv) `(setf (liv-carry ,liv) (remove ,what (liv-carry ,liv))))
...
(define-symbol-macro annsize 128)
(defun make-net ()
...
(defun run-net (net i0 i1 i2 i3 i4 i5)
  (declare (single-float i0 i1 i2 i3 i4 i5) ((array single-float) net))
  (macrolet ((anet (i)
               `(aref net ,i))
             (a* (i j)
    ...
    (loop for i from 0 below 6 do
      (let ((n (+ i 7 (* i 8)))) ; 7 is number of output neurons, 8 is number of weights each neuron uses
        ;(format t "hidden-neuron: ~a~%"
...
(defun die-liv (liv i)
  ...
  (let ((error (liv-annerror liv)))
    (if (has-carry :gold liv) (incf error 16))
    (let ((besterror (case (liv-art liv)
                       (0 *best-liv-annerror-0*)
                       (1 *best-liv-annerror-1*)
                       (2 *best-liv-annerror-2*))))
      (when (>= error besterror)
        (case (liv-art liv)
          (0 (setf *best-liv-net-0* (liv-net liv)) (setf *best-liv-annerror-0* error))
          (1 (setf *best-liv-net-1* (liv-net liv)) (setf *best-liv-annerror-1* error))
          (2 (setf *best-liv-net-2* (liv-net liv)) (setf *best-liv-annerror-2* error))))))
  (setf (aref *pop* i) nil))
...
(defun run-liv (liv i)
  (let* ((x (liv-x liv))
         ...
         (ny 0)
         (nei (neighbours x y)))
    (destructuring-bind (bx by) (case art
                                  (0 (list  4  4))
                                  (1 (list 34  4))
                                  (2 (list 20 20)))
    (if (not ox) (setf ox -1))
    ...
    (assert (= objid i))
    (incf (liv-age liv))
    (when (> (liv-age liv) 256)
      (format t "too old!~%")
      (die-liv liv i)
      (return-from run-liv))
    (decf (liv-energy liv))
    ;(if (get-plague-liv-flag liv) (decf (liv-energy liv)))
    (when (or (< (liv-energy liv) 0) (< (liv-annerror liv) -8))
      (die-liv liv i)
      (return-from run-liv))
    ;(format t "nei: ~s~%" nei)
    (when (has-carry :gold liv)
      (when (and (< (abs (- x bx)) 4) (< (abs (- y by)) 4))
        (del-carry :gold liv)
        (format t "mined art=~a~%" (case art (0 "red") (1 "green") (2 "blue")))
        (incf (liv-energy liv) 128)
        (incf (liv-annerror liv) 128)))
    (destructuring-bind (w e n s) nei
      (let ((ee 0f0) (fe 0f0) (gold 0f0))
        (declare (single-float ee fe))
        ; energy box
        (when (or (= n -2) (= s -2) (= w -2) (= e -2))
          (if (not (has-carry :gold liv))
            (add-carry :gold liv)))
        ...
        (if (has-carry :gold liv) (setf gold 1f0))
        (run-net net (coerce x 'single-float) (coerce y 'single-float) ee fe gold 1f0)
        (if (> (aref net 0) 0.7) (incf nx))
        (if (> (aref net 1) 0.7) (decf nx))
        (if (> (aref net 2) 0.7) (incf ny))
        (if (> (aref net 3) 0.7) (decf ny))
        (if (and (= ox x) (= oy y))
          (decf (liv-annerror liv))
          (let ((od (+ (abs (- ox bx)) (abs (- oy by))))
                (nd (+ (abs (-  x bx)) (abs (-  y by)))))
           (if (and (> gold 0.5f0) (> od nd)) ; if we are carrying gold and getting nearer hqbase, that's good.
             (incf (liv-annerror liv) 2))
           (if (and (> gold 0.5f0) (< od nd)) ; if we are carrying gold and further from hqbase, that's bad.
             (decf (liv-annerror liv)))
           (if (and (< gold 0.5f0) (< od nd)) ; if we are not carrying gold and getting further from hqbase, that's good.
             (incf (liv-annerror liv)))))
;        (when (> (aref net 2) 0.7)
...
;          (when (> s 0) (breed liv (aref *pop* s))))
    ...
    (setf (liv-ox liv) (liv-x liv))
    (setf (liv-oy liv) (liv-y liv))
    (when (or (/= nx 0) (/= ny 0))
      (setf nx (+ x nx))
      (setf ny (+ y ny))
      (when (and (> nx -1) (> ny -1)
    ...
    (loop for i from 1 below poplen do
      (let ((liv (aref *pop* i)))
        (when liv 
          ...
          (run-liv liv i))))
    (format t "stats: size=~a maxgen=~a plague=~a best-annerror: ~,2f/~,2f/~,2f~%" pops maxgen plague *best-liv-annerror-0* *best-liv-annerror-1* *best-liv-annerror-2*))
  ; handle create spore requests
  (let ((lst *create-spore-request*) n)
    (setf *create-spore-request* nil)
    (loop for (art nx ny) in lst do
      (setf n nil)
      (when (= (get-obj nx ny) 0)
        (loop for i from 1 below (length *pop*) do
          (when (not (aref *pop* i))
    ...
    ((when (>= (random 2) 0) ; how often to spawn a spore
       (let* ((x (field-value :column self))
              ...
              (art (cond
                     ...
                     ((and (= x 34) (= y 4)) 1)
                     ((and (= x 20) (= y 20)) 2)
                     (t (error "no base at ~a,~a" x y)))))
         (case (random 4)
           (0 (setf nx (+ x (random 4) 1)) (setf ny y))
           (1 (setf ny (+ y (random 4) 1)) (setf nx x))
           (2 (setf nx (- x (random 4) 1)) (setf ny y))
           (3 (setf ny (- y (random 4) 1)) (setf nx x)))
         (if (< nx 0) (setf nx 0))
         (if (< ny 0) (setf ny 0))
         (if (>= nx *room-mx*) (setf nx (1- *room-mx*)))
         (if (>= ny *room-my*) (setf ny (1- *room-my*)))
         (push (list art nx ny) *create-spore-request*)))))
      ...
      (let* ((spore (clone =spore=))
        ...
        (when (= (get-obj x y) 0)
          (let ((net (case art
                       ...
                       (1 *best-liv-net-1*)
                       (2 *best-liv-net-2*))))
            (if net (copy-net-random (liv-net liv) net net)))
...
;        [drop-edible-wall self r c r (+ column len)])))
  ...
  ; create some gold-mines
  (loop for x from 12 below 28 do
    (loop for y from 10 below 14 do
      (when (>= (random 8) 0)
